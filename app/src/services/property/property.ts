...
export class PropertyService {
    ...
    static async getById(searchId: number, propertyId: number): Promise<{data: Property | undefined}> {
        const url = `${this.propertyApiUrl}/${searchId}/properties/${propertyId}`
        return await axios.get(url)
    }

    static async update(property: UpdatePropertyCommand, propertyId: number, searchId: number): Promise<{data: Property}> {
        const url = `${this.propertyApiUrl}/${searchId}/properties/${propertyId}`
