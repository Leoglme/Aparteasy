<template>
    <Form id="signup-form" v-slot="{meta}" class="grid columns col-1 gap-6 sm:col-2" @submit="signup">
        ...
        <FormInput v-model:value="values.email" placeholder="email@gmail.com" rules="required|email" label="Email"
                   type="email" id="email"/>
        <FormInput v-model:value="values.password" rules="required|min:6" label="Mot de passe" type="password"
                   id="password"/>
        <FormInput v-model:value="values.password_confirmation" rules="required|confirmed:@password"
                   label="Confirmation mot de passe" type="password" id="password_confirmation"/>
        ...
        <h6 class="text-medium text-sm centered sm:justify-start gap-2 flex-wrap">Vous avez déjà un compte ?
            <router-link class="link" :to="{name: 'login', query: $route.query}">Se connecter</router-link>
        </h6>
    </Form>
</template>
