<template>
    <Form v-slot="{meta}" class="flex flex-col gap-6" @submit="handleSubmit">
        ...
        <div class="grid columns sm:col-2 gap-6 items-baseline">
            ...
            <FormInput v-model:value="values.amount_of_charges" label="Montant des charges" id="amount_of_charges"
                       rules="numeric" type="number"/>
        </div>

        <div class="grid columns sm:col-2 gap-6 items-baseline">
            <FormInput v-model:value="values.number_of_rooms" label="Nombre de pièces" id="number_of_rooms"
                       rules="required|numeric" type="number"/>
            <FormInput v-model:value="values.surface_area" id="surface_area" rules="required|numeric" type="number">
                <template #label>
                    <span>Surface <span class="text-xs text-contrast-70">(en m<span
