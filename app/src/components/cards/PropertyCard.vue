<template>
    <RouterLink v-if="props.property"
                :to="{ name: 'property', params: { searchId: searchId, propertyId: props.property.id } }" class="bg-grey-500 flex flex-col search-card h-full
     ...
     <span><span class="text-contrast-70">Charges:</span> {{ props.property.amount_of_charges || 0 }}€</span></span>
        ...
        <span v-if="props.property.availability_date">Disponible le {{
            ...
            <div class="flex flex-col justify-end gap-2 mt-2 h-full">
                ...
                <Button startIcon="link" :href="props.property.url" externalLink @click.prevent="openPropertyUrl"
                        target="_blank">
                    Voir l’annonce
                </Button>
            </div>
